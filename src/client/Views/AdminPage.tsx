...
const AdminPage: React.FC<AdminPageProps> = (props) => {
    ...
    React.useEffect(() => {
        (async () => {
            const res = await apiService('/api/tags');
    ...
    React.useEffect(() => {
        (async () => {
            const res = await apiService(`/api/blogs/${blogid}`);
            const blog = await res.json();

            const res2 = await apiService(`/api/blogtags/${blogid}`);
    ...
    const handleEdit = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const res = await apiService(`/api/blogs/${blogid}`, 'PUT', {title, content});
        const resultBlog = await res.json();
        console.log(resultBlog);
        //@ts-ignore
        if (oldID !== Number(selectedTag)) {

            const res2 = await apiService(`/api/blogtags/${blogid}`,'PUT',{ title, content });
            const res = await apiService(`/api/blogs/${blogid}`,'PUT',{ title, content });
    ...
    const handleDelete = async (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        const res = await apiService(`/api/blogtags/${blogid}`, 'DELETE');
        const res2 = await apiService(`/api/blogs/${blogid}`,'DELETE' );
        if (res.ok && res2.ok) {
            history('/');
        }
    }
